<template>
    <div class="approval-process">
        ...
        <process-list :reset-status="resetStatus" :search-data="searchData" :loading="loading" :back-data="backData" :data-list="dataList" @handle-page="handlePage" @reload-page="reloadPage"></process-list>
    </div>
</template>
<script>
    ...
    methods: {
         ...
         */
         async dealPageOrPageSize(params) {
            ...
            let res;
            res = await $api.getProcessApproval(JSON.stringify(params));
        ...
        async reloadPage() {
            let params = JSON.parse(JSON.stringify(this.$route.query));
            await this.getProcessCreated(params);
        }
    }
}
</script>
<style lang="less">
@import './processApproval.less';
</style>