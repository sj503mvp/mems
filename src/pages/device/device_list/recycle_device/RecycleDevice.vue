<template>
    <div class="recycle-device">
        <search-option :reset-status="resetStatus" @on-search="onSearch" @on-clear="onClear" :search-data="searchData"></search-option>
        <recycle-device-list :reset-status="resetStatus" :search-data="searchData" :loading="loading" :back-data="backData" :data-list="dataList" @handle-page="handlePage"></recycle-device-list>
    </div>
</template>
<script>
...
import RecycleDeviceList from '../business/List.vue'
...
export default {
    ...
    methods: {
        ...
        getRecycleDevice: debounce(async function(params) {
            this.loading = true;
         ...
         */
         async dealPageOrPageSize(params) {
            ...
            let res;
            res = await $api.getRecycleDevice(params);
